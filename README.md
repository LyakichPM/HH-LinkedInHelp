...
| `li_can_message.py` | Проверка ДО написания текста: открыта ли личка (1-й круг) или доступен только платный InMail |
| `li_person_actions.py` | Разбор карточки профиля: какие действия вообще доступны, включая спрятанные под «Ещё» |
| `li_msg_from_profile.py` | DM прямо из карточки профиля, с проверкой шапки треда перед вводом |
| `li_jobs_search.py` | Поиск вакансий по запросу с выгрузкой в JSON |
| `li_jobs_merge.py` | Свод выдач поиска в один список с раскладкой по ярусам |
| `li_job_view.py` | Полный текст вакансии и её метаданные по `jobId` |
| `li_job_apply.py` | Easy Apply: `dry` печатает поля и кнопки и ничего не отправляет, `go` подаёт заявку с резюме |
| `li_posts_search.py` | Поиск постов с вакансиями (их не видно в разделе Jobs) |
| `li_posts_merge.py` | Свод найденных постов с отсевом повторов |
...
Репозиторий публичный, а инструмент работает с живыми сессиями hh.ru и
LinkedIn и с личными данными соискателя. Поэтому:

- `.gitignore` устроен **белым списком**: корень закрыт целиком (`/*`),
  открыт поимённо только инструментарий. Новый разовый скрипт, дамп вакансии
  или скриншот не утекут из-за того, что под них не написали правило.
- Доступы (`config.local.json`, `*_cookies.json`, `*.bak`, профили браузера
  `data/`, `*_profile/`) в репозиторий не попадают никогда.
- Личные данные владельца (ФИО, телефон, телеграм, id резюме) лежат в
  `agent/owner.local.md`, он в игноре; в публичных инструкциях - плейсхолдеры.
- Проверка перед публикацией: `py tools/audit_pii.py --out audit.txt`. Она
  сканирует ровно тот набор файлов, который уйдёт в коммит, и возвращает
  ненулевой код, если нашла доступы.
- Автоматика: `gitleaks` в pre-commit и в CI по всей истории, плюс
  `check-added-large-files` (1 МБ) - чтобы профиль браузера не уехал незаметно.

Подробности и разбор реальных находок - в [docs/security.md](docs/security.md).
